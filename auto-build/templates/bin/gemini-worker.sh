...
LOG_FILE="$ROOT/logs/workers/${TASK_ID}.gemini.log"
RESULT_FILE="$RESULT_DIR/${TASK_ID}.gemini.result.json"
META_FILE="$RESULT_DIR/${TASK_ID}.gemini.meta.json"
...
(
  cd "$WORKTREE_DIR"
  gemini -p "$WORKER_PROMPT" --output-format json --approval-mode yolo >"$RESULT_FILE" 2>"$LOG_FILE"
)
EXIT_CODE=$?
set -e
